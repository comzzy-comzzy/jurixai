...
contract JuriXEscrow {
    ...
    event EscrowRegistered(string hackathonId, uint256 prizePool, uint256 platformFee);
    ...
    function registerHackathon(
        ...
        hackathons[hackathonId] = Hackathon(prizePool, platformFee, false, true);

        if (platformFee > 0) {
            require(IERC20(usdcToken).transfer(feeCollector, platformFee), "Platform fee transfer failed");
        }

        emit EscrowRegistered(hackathonId, prizePool, platformFee);
    }
    ...
    function disbursePrizes(
        ...
        require(!h.disbursed, "Already disbursed");
...
async function main() {
  ...
  console.log(`Compilation complete. Deploying contract to ${activeChain.name}...`);
  ...
  console.log(`\n🎉 JuriXEscrow contract deployed at: ${contractAddress}\n`);
